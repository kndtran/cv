...
\begin{innerlist}
	...
	\begin{innerlist}
		...
		\item[$-$] Developed a chatbot interaction interface with unique background coordination methods between the front-end and Watson Assistant.
    \item[$-$] Developed text analytics pipelines for clients in Watson Studio.
    \item[$-$] Other research work includes analysis of USPTO patents and detection of malicious URLs.
    \item[$-$] Patents (USPTO): 1 patent pending, 1 pending submission, 3 in drafting.
    \item[$-$] Defensive Patent Publications (IP.com): 2 published.
		%\item[$-$] 2 publications.
  \end{innerlist}
  ...
  \halfblankline
    ...
    \item[$-$] Patents: 2 patents pending. %\cite{Tran2018a} (pending), \cite{Tran2018b} (pending).
    \item[$-$] Defensive Patent Publications (IP.com): 2 published.
		%\item[$-$] 2 publications.
  \end{innerlist}
\end{innerlist}
...
\item Bachelor of Computer Science, with First Class Honours. \hfill{Feb 2006 -- Dec 2009}
	\begin{innerlist}
		...
		\begin{innerlist}
			...
			\item[$*$] 2 Publications. %: \cite{Compton2009}, \cite{Tran2009}, \cite{Tran2009a}.
			%\item[$*$] 3 publications.
		\end{innerlist}
		\item[$-$] Individual Research Projects: \emph{Detecting Network Anomalies}.
		\begin{innerlist}
			\item[$*$] Supervisor: Dr. Huidong (Warren) Jin.
			\item[$*$] 2 Publications. %: \cite{Tran2009b}, \cite{Tran2010}.
			%\item[$*$] 2 publications.
		\end{innerlist}
	\end{innerlist}
\end{innerlist}
